...
// Primeiro boot do AVD do projeto pode passar de 3 min
...
while (Date.now() - inicio < limiteMs) {
  ...
  if (linha) {
    ...
    if (boot === '1') {
      udidOk = udid;
      console.log(`[ok] Emulador do projeto pronto: ${udid}`);
      pronto = true;
      break;
    }
  }
...
if (!pronto) {
  console.error('[erro] Timeout aguardando boot do emulador do projeto.');
  process.exit(1);
}
